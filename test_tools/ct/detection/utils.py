...
def get_clips_uniform(path, count, clip_size):
    ...
    start_indices = np.linspace(0, final_start, count, endpoint=True, dtype=np.int32)
...
def get_bbox(detect_res):
    ...
    all_face_bboxs = np.array(all_face_bboxs).astype(np.int32)
